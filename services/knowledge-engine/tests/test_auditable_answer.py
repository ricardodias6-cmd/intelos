...
@pytest.mark.asyncio
async def test_regeneration_preserves_valid_claims(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    candidates = [
        CandidateAnswer(
            answer="Resposta mista.",
            claims=[
                CandidateClaim(
                    text="A autorização compete à entidade competente.",
                    evidence_ids=["EV_ONE"],
        ...
        CandidateAnswer(
            ...
            claims=[
                ...
                CandidateClaim(
                    text="A entidade deve confirmar o procedimento.",
                    evidence_ids=["EV_ONE"],
                ),
            ],
        ),
    ...
    assert provision_calls == 2
    assert result.status == "answered"
    assert "A autorização compete à entidade competente." in result.answer
    assert "A entidade deve confirmar o procedimento." in result.answer
    assert all(
